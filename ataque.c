...
//atacadas sin tomar en cuenta al rey y sin los peones
...
//Se obtiene las piezas que son atacadas por la pieza ubicada en el escaque (usada en xmm)
uint64 obtMapaAtacadasPorPieza_Color(int escaque, COLOR color)
{

	uint64 atacadas, mapaPiezas;

	atacadas 	^= atacadas;
 	mapaPiezas	^= mapaPiezas;

	if (color)
	{
		mapaPiezas = juego.negros ^ juego.tablero[NEGRO][REY];		
	} else {
		mapaPiezas = juego.blancos ^ juego.tablero[BLANCO][REY]; 					\
	}


	switch (ESCAQUES[escaque]) 
	{
		case PEON_BLANCO:
		case PEON_NEGRO: 
			atacadas = (mascaraCapturarPeon[escaque][color] & mapaPiezas);
			break;

		case CABALLO_BLANCO:
		case CABALLO_NEGRO:
			atacadas = (mascaraCaballo[escaque] & mapaPiezas);
			break;

		case ALFIL_BLANCO:
		case ALFIL_NEGRO:
			atacadas = (genAlfilMOVAtaqueTablero(escaque, juego) & mapaPiezas);
			break;

		case TORRE_BLANCO:
		case TORRE_NEGRO:
			atacadas = (genTorreMOVAtaqueTablero(escaque, juego) & mapaPiezas);
			break;

		case DAMA_BLANCO:
		case DAMA_NEGRO:
			atacadas = ((genTorreMOVAtaqueTablero(escaque, juego)  |
				     genAlfilMOVAtaqueTablero(escaque, juego)) & mapaPiezas);
			break;
