...
{
	if (STATUS_FIN_JUEGO_TERMINADO) return 0;
...
//********************************************************************************************
	...
	if (esConsola && xboardVerDetalles)
	{
...
//		profundidadPrint = profundidadActual;
		...
		if (xboardVerDetalles)
		{
			...
			{
				printf("%d %d %llu %d ",profundidadActual, valoracion, 
						tiempo/1000,(contadorNodos+QcontadorNodos));
		...
		if (esConsola && xboardVerDetalles)
		{
			printf("%3d %10lld  %9d  %9d  %6d     ",profundidadActual, 
						tiempo,contadorNodos,QcontadorNodos, valoracion);
			for (i_pv=0;i_pv<juego.triangularLargo[0];++i_pv)
			{
				imprimirMovimiento(juego.triangularArray[0][i_pv]);
				printf("  ");
 ...
 *************************************************************************/
			...
			if (V > alfa)
			{
				...
				if (juego.colorTurno)
				{ 
					juego.heuristicaNegra[OBT_MOV_ORIGEN(juego.Buffer_MOV[i])][posDestinoMovPeon] += profundidad*profundidad;

					if (maxHistorial[NEGRO] < juego.heuristicaNegra[OBT_MOV_ORIGEN(juego.Buffer_MOV[i])][posDestinoMovPeon])
					{
						maxHistorial[NEGRO] = juego.heuristicaNegra[OBT_MOV_ORIGEN(juego.Buffer_MOV[i])][posDestinoMovPeon];
					}

				} else { 

					juego.heuristicaBlanca[OBT_MOV_ORIGEN(juego.Buffer_MOV[i])][posDestinoMovPeon] += profundidad*profundidad;

					if (maxHistorial[BLANCO] < juego.heuristicaBlanca[OBT_MOV_ORIGEN(juego.Buffer_MOV[i])][posDestinoMovPeon])
					{
						maxHistorial[BLANCO] = juego.heuristicaBlanca[OBT_MOV_ORIGEN(juego.Buffer_MOV[i])][posDestinoMovPeon];
					}
				}
