...
static inline uint32 bitScanForwardBruijn(uint64 bb) 
{
	static const uint32 index64[64] 	= {
...
static inline int convertir2lista(uint64 bb, uint8 *lista)
{
	int total=0;
	while ( bb ) {
	   *lista++  = (uint8)bitScanForwardBruijn(bb); // determine bit index of least significant one bit
	   total++;	
	   bb &= bb-1; // reset LS1B
	}
