...
#define 	VERSION_MANGO_AJEDREZ 		"Mango Paola Ajedrez 1.5, (c) 2016. Creado por: Jose Andres Morales Linares. comprasmangocomputer@gmail.com"

#define NOMBRE_MANGO_PAOLA			"Mango-Paola-Ajedrez"
#define VERSION_MANGO_PAOLA			"1.5"
...
#define		NTOTAL_PIEZAS_NEGRAS_SR	(cuentaBit(juego.tablero[NEGRO][PEON])    		+ \
			    	  	 	cuentaBit(juego.tablero[NEGRO][DAMA])    				+ \
				  		 	cuentaBit(juego.tablero[NEGRO][CABALLO]) 				+ \
				  	 		cuentaBit(juego.tablero[NEGRO][TORRE])   				+ \
				  		 	cuentaBit(juego.tablero[NEGRO][ALFIL]))

#define		OBT_COLOR_PIEZA(pieza) ((pieza < 9) ? BLANCO:NEGRO)
#define		ES_RANGO_POS_VALIDA(pos) (pos < 64)
#define		ES_POS_VACIO(pos) (ESCAQUES[pos]==VACIO)

#define		ES_ESTA_ON_MINIJUEGO(nm) (nm>0 && nm<24)
/*
#define		ES_ESTA_ON_MINIJUEGO(nm) ((nm==1) || (nm==2) || (nm==3) || (nm==4) || (nm==5) || (nm==6) || (nm==7) || (nm==8) ||(nm==9) || (nm==10) || (nm==11) || (nm==12) || (nm==13) || (nm==14) || (nm==15) || (nm==16) || (nm==17) || (nm==18) || (nm==19) || (nm==20) || (nm==21) || (nm==22) || (nm==23))
*/
#define		NPROM "XMM> "
...
//*********************************** ES_INSUFICIENTE_MATERIAL_LADO_BLANCO(es) ************************************************ 
#define		ES_INSUFICIENTE_MATERIAL_LADO_BLANCO(es) 																	\
	es = FALSO;																											\
	if (!juego.tablero[BLANCO][PEON]) /* No hay peones en el tablero	*/												\
	{																													\
		/* insuficiente material*/																						\
		if ((juego.material_lado_blanco < VALOR_TORRE))																	\
		{																												\
			es = VERDADERO;																								\
																														\
		} 																												\
...
//*********************************** ES_INSUFICIENTE_MATERIAL_LADO_NEGRO(es) ************************************************ 
#define		ES_INSUFICIENTE_MATERIAL_LADO_NEGRO(es) 																	\
	es = FALSO;																											\
	if (!juego.tablero[NEGRO][PEON]) /* No hay peones en el tablero	*/													\
	{																													\
		/* insuficiente material*/																						\
		if ((juego.material_lado_negro < VALOR_TORRE))																	\
		{																												\
			es = VERDADERO;																								\
																														\
		} 																												\
...
#define		STATUS_FIN_JUEGO_TABLAS_ACORDADAS			10
...
#define MAX_MAPAS_MINIJUEGO23	12
#define LARGO_BUFER_IMPRIMIR_MOV_XMM 128
#define SALIDA_LABERINTO 69 //Codigo Ascii de la Letra E, E de Exit
#define W_MURO 87
#define w_MURO 119

#define		NTOTAL_PIEZAS_BLANCAS_SR (cuentaBit(juego.tablero[BLANCO][PEON])    + \
			    	  	  	cuentaBit(juego.tablero[BLANCO][DAMA])    			+ \
				  	 	 	cuentaBit(juego.tablero[BLANCO][CABALLO]) 			+ \
				  	 		cuentaBit(juego.tablero[BLANCO][TORRE])   			+ \
				  	  		cuentaBit(juego.tablero[BLANCO][ALFIL]))


#define		NTOTAL_PIEZAS_BLANCAS_SR_SP (cuentaBit(juego.tablero[BLANCO][DAMA])   	+ \
				  	     cuentaBit(juego.tablero[BLANCO][CABALLO]) 					+ \
				  	     cuentaBit(juego.tablero[BLANCO][TORRE])   					+ \
				  	     cuentaBit(juego.tablero[BLANCO][ALFIL]))

#define		NTOTAL_PIEZAS_NEGRAS_SR_SP (cuentaBit(juego.tablero[NEGRO][DAMA])    	+ \
				  	    cuentaBit(juego.tablero[NEGRO][CABALLO]) 					+ \
				  	    cuentaBit(juego.tablero[NEGRO][TORRE])   					+ \
				  	    cuentaBit(juego.tablero[NEGRO][ALFIL]))
